...
func (s *TransactionService) RemoveFromCart(
	...
	// Load the order first to get its integer ID.
	existing, err := s.loadOrderBySession(ctx, req.Msg.SessionId)
	if err != nil {
		return nil, connect.NewError(connect.CodeNotFound, errors.New("cart not found"))
	}

	err = s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		result := tx.Delete(&models.OrderItem{}, "order_id = ? AND product_id = ?", existing.ID, req.Msg.ProductId)
		if result.Error != nil {
			return result.Error
		}
		return s.recalcTotal(tx, existing.ID)
	})
	if err != nil {
